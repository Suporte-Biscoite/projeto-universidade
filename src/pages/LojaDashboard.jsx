...
import {
  Users, TrendingUp, CheckCircle, Clock, BookOpen,
  AlertTriangle, Home, LogOut, BarChart2, Search, X, ChevronRight,
} from 'lucide-react';
import { useProfile, STORES } from '../context/ProfileContext';

// Simula a loja do líder logado (em produção viria do JWT)
const CURRENT_STORE_ID = 'loja_eldorado';
...
export default function LojaDashboard() {
  ...
  const store = STORES.find(s => s.id === CURRENT_STORE_ID) || { name: 'Loja', city: '', state: '' };
  const employees = users.filter(u => u.systemRole === 'aluno' && u.storeId === CURRENT_STORE_ID);
    ...
    <div className="min-h-screen bg-[#f6f9fd] flex">
      ...
      <aside className="w-64 min-h-screen bg-white border-r border-slate-100 flex flex-col py-8 px-6 fixed left-0 top-0 bottom-0 z-30">
        <div className="mb-10 text-center">
          ...
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2">Painel da Loja</p>
        </div>

        <div className="mb-6 px-1">
          <p className="text-base font-black text-[#001A26] leading-tight">{store.name}</p>
          <p className="text-xs text-slate-400 mt-0.5">{store.city} · {store.state}</p>
          <div className="mt-2 inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-600 text-[10px] font-black px-2 py-1 rounded-full">
      ...
      <main className="ml-64 flex-1 p-10 space-y-10 max-w-[1100px]">
        ...
        <div>
          <h1 className="text-3xl font-black text-[#001A26]">{store.name}</h1>
          <p className="text-slate-400 text-sm mt-1">{store.city} — {total} colaboradores cadastrados</p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard icon={Users}       label="Colaboradores"   value={total}          sub={`${activeCount} ativos`}         color="bg-blue-50 text-blue-500" />
          ...
          <StatCard icon={AlertTriangle} label="Sem progresso" value={employees.filter(e => { const p = EMPLOYEE_PROGRESS[e.id]; return p && p.coursesCompleted === 0; }).length} sub="precisam de atenção" color="bg-orange-50 text-orange-500" />
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-[24px] border border-slate-100 shadow-sm overflow-hidden">
              <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                <p className="font-black text-[#001A26]">Ranking de progresso</p>
                <button
                  onClick={() => setActiveTab('employees')}
                  className="text-xs font-bold text-[#4A72B2] hover:underline flex items-center gap-1"
                >
              ...
              <div className="divide-y divide-slate-50">
                {[...employees]
                  .sort((a, b) => {
                    const pa = EMPLOYEE_PROGRESS[a.id];
                    const pb = EMPLOYEE_PROGRESS[b.id];
                    const ra = pa ? pa.coursesCompleted / pa.coursesTotal : 0;
                    const rb = pb ? pb.coursesCompleted / pb.coursesTotal : 0;
                    return rb - ra;
                  })
                  .slice(0, 5)
                  .map((emp, idx) => {
                    const prog = EMPLOYEE_PROGRESS[emp.id] || { coursesCompleted: 0, coursesTotal: 5, avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=200', role: 'Colaborador' };
                    return (
                      <div key={emp.id} className="px-6 py-4 flex items-center gap-4">
                        <span className={`text-xs font-black w-6 text-center ${idx === 0 ? 'text-yellow-500' : idx === 1 ? 'text-slate-400' : idx === 2 ? 'text-orange-400' : 'text-slate-300'}`}>
                          #{idx + 1}
                        </span>
                        <img src={prog.avatar} className="w-9 h-9 rounded-full object-cover flex-shrink-0" alt={emp.name} />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold text-[#001A26] truncate">{emp.name}</p>
            ...
            <div className="bg-white rounded-[24px] border border-slate-100 shadow-sm p-6 space-y-4">
              <p className="font-black text-[#001A26]">Colaboradores que precisam de atenção</p>
              {employees.filter(e => {
                const p = EMPLOYEE_PROGRESS[e.id];
                return p && p.coursesCompleted === 0;
              }).map(emp => {
                const prog = EMPLOYEE_PROGRESS[emp.id];
                return (
                  <div key={emp.id} className="flex items-center gap-3 py-3 border-t border-slate-100 first:border-0 first:pt-0">
                    <div className="w-8 h-8 rounded-full bg-red-50 flex items-center justify-center flex-shrink-0">
                      <AlertTriangle size={14} className="text-red-400" />
                    </div>
                    <img src={prog?.avatar} className="w-8 h-8 rounded-full object-cover" alt={emp.name} />
                    <div className="flex-1">
                      <p className="text-xs font-bold text-[#001A26]">{emp.name}</p>
                      <p className="text-[10px] text-slate-400">Nenhum curso iniciado · Última atividade: {prog?.lastActivity}</p>
                    </div>
                    <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${emp.active ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
                      {emp.active ? 'Ativo' : 'Inativo'}
                    </span>
                  </div>
                );
              })}
              {employees.filter(e => { const p = EMPLOYEE_PROGRESS[e.id]; return p && p.coursesCompleted === 0; }).length === 0 && (
                <p className="text-xs text-slate-400">Nenhum colaborador sem progresso.</p>
              )}
            </div>

            {/* Progresso por curso */}
            <div className="bg-white rounded-[24px] border border-slate-100 shadow-sm p-6 space-y-5">
              <p className="font-black text-[#001A26]">Progresso por curso</p>
              {['Fase 1 - Básico', 'Operação cafeteria', 'Páscoa 2026', 'Atendimento ao cliente', 'Marketing Digital'].map((course, ci) => {
                const completed = Math.round(total * [0.8, 0.55, 0.3, 0.65, 0.45][ci]);
                const pct = total > 0 ? Math.round((completed / total) * 100) : 0;
                return (
                  <div key={course} className="space-y-1.5">
                    <div className="flex justify-between text-xs">
                      <span className="font-bold text-[#001A26]">{course}</span>
        ...
        {activeTab === 'employees' && (
          <div className="space-y-5">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-slate-400 font-bold px-3">{displayedEmployees.length} resultado{displayedEmployees.length !== 1 ? 's' : ''}</span>
            </div>
                  ...
                  {displayedEmployees.map(emp => {
                    ...
                    return (
                      <tr key={emp.id} className="border-t border-slate-50 hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-4 py-4">
                          <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${emp.active ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-slate-100 text-slate-400'}`}>
                            {emp.active ? 'Ativo' : 'Inativo'}
                          </span>
                        </td>
                      </tr>
